...
if ( ! function_exists('build_tree'))
{
    ...
    function build_tree($group, $attributes = array(), &$tree = array())
    {
        if (empty($tree))
        {
            ...
            $get_all_groups=$CI->adjacency_list->get_all($group);
            $tree = parse_children($get_all_groups);
        }
...
if ( ! function_exists('build_tree_item'))
{
    ...
    function build_tree_item($group, $item_id, $attributes = array(), &$tree = NULL, &$output_tree = array())
    {
        ...
        if ( ! empty($tree))
        {
            ...
            $reversed_array = array_reverse($output_tree);

            return format_tree($parsed_children, $reversed_array, $attributes);
        }
